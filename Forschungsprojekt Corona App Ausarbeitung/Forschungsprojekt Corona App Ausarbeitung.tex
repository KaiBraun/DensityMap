...
Der Corona Warn App zugrunde liegendes Feature ist das "Exposure Notification Framework", 
...
Die RPI's werden nun unter den Smartphones im Falle einer Begegnung mithilfe von BLE ausgetauscht. Neben den RPI werden zudem verschlüsselt Informationen über die Begegnungsdauer, 
, sowie dem ungefähren Abstand ausgetauscht.
...
So werden beispielsweise Smartphones verwendet, um die Luftverschmutzung in verschiedenen Stadtteilen von Großstädten oder in Gebäuden zu messen (1). 
Die Standortbasierten Daten werden dann auf einen Server geladen, damit Nutzer die aktuelle Belastung in den Regionen (oder auch Räumen in Gebäuden) abfragen kann. Eine ähnliche Umsetzung gibt es hinsichtlich der Lärmbelästigung (2). 
...
Saremi et. Al. (5) Benutzen unter anderem GPS Daten von Smartphones oder Fahrzeugen, um Informationen über die Existenz von Ampeln an Kreuzungen zu extrahieren. \\
\\
...
Tschuhiya et al. beschäftigen sich mit der allgemeinen Nutzbarkeit des BLE Features zum Erstellen von Maps. 
...
Diese Daten wurde dann per BLE an die Smartphones weitergegeben, wo sie ausgewertet und dann auf einen Server hochgeladen wurden. 

In (3) werden Smartphones und vorinstallierte Raspberry Pies mit CICN (Community Information-Centric Networking)  verwendet, um eine IoT Sensoren Map innerhalb geschlossener Gebäuden, wie etwa Shopping Malls, zu erstellen. 
...
Basalamah (4) beschäftigt sich mit Bluetooth Low Energy Tags zum Aufzeichnen von Bewegungsströmen größerer Menschenmassen. 
...
Unser Ansatz hingegen ist, per BLE möglichst anonym die Daten zu erfassen und damit eine Density-Map zu erstellen. 
Außerdem verwenden wir keine speziellen BLE Tags sondern die Smartphones selbst zum Datenaustausch. \\
\\
Ein Feature, das unserer Density-Map am nächsten kommt sind vermutlich die live Stoßzeiten vom Unternehmen Google.  
...
Für die Umsetzung ihres Features benutzt Google voraussichtlich die personalisierten Nutzerdaten seiner User, wie beispielsweise den Standortverlauf der Google Maps Anwendung. 
